![ScreenShot](https://github.com/pubnative/pubnative-android-mediation-sdk/blob/master/PNLogo.png)
...
```
compile 'net.pubnative:mediation:1.4.5-BETA'
```
...
By default, resources would be cached before serving ad. You can set it to false as follows in order to use urls to download resources like icon, banner etc.
```java
request.setCacheResources(false);
```
This needs to be set before calling `start`.
...
ad.withTitle(<TITlE_VIEW>)
  ...
  .withAdvertisingDisclosureView(<CONTEXT>, <ADVERTISING_DISCLOSURE_VIEW>)
